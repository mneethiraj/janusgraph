...
public class JanusColumnDao extends BaseDao<JanusColumn> {
    ...
    public void addOrUpdate(long keyId, byte[] name, byte[] value) {
        JanusColumn column = getColumn(keyId, name);
        ...
        if (column != null) {
            ...
            super.create(new JanusColumn(keyId, name, value));
        }
    }
    ...
    public byte[] getValue(long keyId, byte[] name) {
        try {
            Object result = em.createNamedQuery("JanusColumn.getValueByKeyIdName")
                              .setParameter("keyId", keyId)
                              .setParameter("name", name)
                              .getSingleResult();
    ...
    public JanusColumn getColumn(long keyId, byte[] name) {
        JanusColumn ret = null;

        try {
            ret = em.createNamedQuery("JanusColumn.getColumnByKeyId_Name", JanusColumn.class)
                    .setParameter("keyId", keyId)
                    .setParameter("name", name)
                    .getSingleResult();
        } catch (javax.persistence.NoResultException excp) {
    ...
    public List<JanusColumnValue> getColumns(long keyId) {
        List<Object[]> result = em.createNamedQuery("JanusColumn.getColumnsByKeyId")
                                  .setParameter("keyId", keyId)
                                  .getResultList();
    ...
    public Map<StaticBuffer, List<JanusColumnValue>> getKeys(long storeId) {
        List<Object[]> result = em.createNamedQuery("JanusColumn.getColumnsByStoreId")
    ...
    public Map<StaticBuffer, List<JanusColumnValue>> getKeysByColumnRange(long storeId, byte[] startColumn, byte[] endColumn, int limit) {
        List<Object[]> result = em.createNamedQuery("JanusColumn.getColumnsByStoreId_ColumnRange")
                                  .setParameter("storeId", storeId)
                                  .setParameter("startName", startColumn)
                                  .setParameter("endName", endColumn)
                                  .setMaxResults(limit)
    ...
    public Map<StaticBuffer, List<JanusColumnValue>> getKeysByKeyAndColumnRange(long storeId, byte[] startKey, byte[] endKey, byte[] startColumn, byte[] endColumn, int limit) {
        List<Object[]> result = em.createNamedQuery("JanusColumn.getColumnsByStoreId_KeyRange_ColumnRange")
                                  .setParameter("storeId", storeId)
    ...
    private List<JanusColumnValue> toColumnList(List<Object[]> result) {
        ...
        if (result != null && !result.isEmpty()) {
            ...
            for (Object[] row : result) {
                ...
                byte[] value = toByteArray(row[1]);

                ret.add(new JanusColumnValue(name, value));
            }
        }
